...
(function(root, factory) {
	...
	function oauthConfig(${symbol_dollar}httpProvider) {
		${symbol_dollar}httpProvider.interceptors.push("oauthInterceptor");
	}
	oauthConfig.$inject = [ "${symbol_dollar}httpProvider" ];
	function oauthInterceptor($q, ${symbol_dollar}rootScope, OAuthToken) {
		return {
			...
			responseError: function(rejection) {
				if (400 === rejection.status && rejection.data && ("invalid_request" === rejection.data.error || "invalid_grant" === rejection.data.error)) {
					...
					${symbol_dollar}rootScope.$emit("oauth:error", rejection);
				}
	...
	oauthInterceptor.$inject = [ "$q", "${symbol_dollar}rootScope", "OAuthToken" ];
	...
	function OAuthProvider() {
		...
		this.$get = function(${symbol_dollar}http, OAuthToken, ${symbol_dollar}base64, ${symbol_dollar}location) {
			var OAuth = function() {
				function OAuth() {
					if (!config) {
				...
				_prototypeProperties(OAuth, null, {
					...
					getAccessTokenFromCode: {
						value: function getAccessTokenFromCode(options) {
							...
							if (${symbol_dollar}location.$$html5) {
								code = ${symbol_dollar}location.search().code;
							} else {
							...
							options = angular.extend({
								headers: {
									...
									"Authorization": "Basic " + ${symbol_dollar}base64.encode(config.clientId + ":" + config.clientSecret)
								}
							}, options);

							return ${symbol_dollar}http.post("" + config.baseUrl + "" + config.grantPath, data, options).then(function(response) {
								OAuthToken.token = response.data;

								setTimeout(function() {
									if (${symbol_dollar}location.$$html5) {
										${symbol_dollar}location.search({});
									} else {
										location.href = '/';
									}
								}, 200);
					...
					revokeToken: {
						value: function revokeToken() {
							...
							return ${symbol_dollar}http.post("" + config.baseUrl + "" + config.revokePath, data, options).then(function(response) {
								OAuthToken.removeToken();
								return response;
							});
						},
		...
		this.$get.$inject = [ "${symbol_dollar}http", "OAuthToken", "${symbol_dollar}base64", "${symbol_dollar}location" ];
	}
	var _prototypeProperties = function(child, staticProps, instanceProps) {
		if (staticProps) Object.defineProperties(child, staticProps);
		if (instanceProps) Object.defineProperties(child.prototype, instanceProps);
